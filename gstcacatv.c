...
* |[
* gst-launch-1.0 -v videotestsrc ! cacatv ! videoconvert ! autovideosink
...
#define GST_CACA_DEFAULT_FONT 1
...
gst_cacatv_transform_frame (GstVideoFilter * vfilter, GstVideoFrame * in_frame,  GstVideoFrame * out_frame)
{
	GstCACATv *cacatv = GST_CACATV (vfilter);
	
	GST_OBJECT_LOCK (cacatv);
	caca_clear_canvas(cacatv->canvas);
	caca_dither_bitmap(cacatv->canvas,0,0,caca_get_canvas_width(cacatv->canvas), caca_get_canvas_height(cacatv->canvas),cacatv->dither, GST_VIDEO_FRAME_PLANE_DATA (in_frame, 0));
	//libcaca always renders ARGB
	caca_render_canvas(cacatv->canvas,cacatv->font, GST_VIDEO_FRAME_PLANE_DATA (out_frame,0), cacatv->src_width , cacatv->src_height , 4*cacatv->src_width);
...
static gboolean
gst_cacatv_setcaps(GstVideoFilter *filter, GstCaps *incaps, GstVideoInfo *in_info,GstCaps *outcaps, GstVideoInfo *out_info)
{	
	...
	cacatv->dither = caca_create_dither(bpp,cacatv->sink_width,cacatv->sink_height,depth*cacatv->sink_width,red_mask,green_mask,blue_mask,0x00000000);
	caca_set_canvas_size(cacatv->canvas,cacatv->canvas_width,cacatv->canvas_height);
...
//override transform_caps, use sink caps on src, except change format to match libcaca and adjust resolution
static GstCaps* 
gst_cacatv_transform_caps(GstBaseTransform *trans, GstPadDirection direction, GstCaps *caps, GstCaps *filter)
{
	GstCaps *ret;
	...
	if(direction==GST_PAD_SINK ){
		//src the same caps as input into the plugin, except for always in ARGB
		ret = gst_caps_copy(caps);
		
		g_value_init (&src_width, G_TYPE_INT);
		g_value_init (&src_height, G_TYPE_INT);
		//calculate output width from canvas size and font width
		cacatv->src_width = cacatv->canvas_width * caca_get_font_width(cacatv->font);
		cacatv->src_height = cacatv->canvas_height * caca_get_font_height(cacatv->font);
...
gst_cacatv_class_init (GstCACATvClass * klass)
{
	...
	"The width of the canvas in characters", 0, G_MAXINT, GST_CACA_DEFAULT_SCREEN_WIDTH,
	...
	g_param_spec_int ("font", "Font",
	"selected libcaca font", 0, G_MAXINT,
	GST_CACA_DEFAULT_SCREEN_HEIGHT,
	G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
	...
	gst_element_class_add_static_pad_template (gstelement_class_tv, &sink_template_tv);
	gst_element_class_add_static_pad_template (gstelement_class_tv, &src_template_tv);
	
	videofilter_class->transform_frame = GST_DEBUG_FUNCPTR (gst_cacatv_transform_frame);
	videofilter_class->set_info = GST_DEBUG_FUNCPTR (gst_cacatv_setcaps);	
	transform_class->transform_caps = GST_DEBUG_FUNCPTR(gst_cacatv_transform_caps);
}
...
gst_cacatv_init (GstCACATv * cacatv)
{
	...
	cacatv->canvas = caca_create_canvas(cacatv->canvas_width,cacatv->canvas_height);
...
static void
gst_cacatv_set_property (GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static void
gst_cacatv_get_property (GObject * object, guint prop_id, GValue * value,  GParamSpec * pspec)
{	
	GstCACATv *cacatv = GST_CACATV (object);
